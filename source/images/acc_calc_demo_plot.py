...
def create_fitness_block(name, row):
    bd = block(name, text_margin=p(0.3, 0.5), alignment="nw", dotted=True, group='tight', group_margin=[p(1,2), p(0,0)])
    for i in range(3):
        bd += cell(row[i])
...
def plot_calculator(dm, w, highlight, list_len, margin=[p(3,2), p(1,0.5)]):
    ...
    if list_len:
        ...
        cls_net = net()
        for i in range(5):
            leaf_net += path(cl[3].e(1), poffx(2), acc[i].w(2), routedef='|-', style=('', '>'))
            cls_net += path(cl[3].e(2), poffx(3), acc[i].w(3), routedef='|-', style=('', '>'))
    ...
    if list_len:
        ...
        img += block(size=p(0.2, cl.size[1]+0.1), fill='white', border=False).align(cl.n() - p(0.1, 0.05))
        img += text("Classifier Output").align(cl.s(), cur().n())
    ...
    acc += path(acc[0][0].w(), acc[4][0].w(1.0)+p(0,2), dotted=True)
    for i in range(3):
        acc += text("$C_{}$".format(i+1)).align(acc[4][i].s(0.5) + p(0,0.5), cur().n(0.5))
        acc += path(acc[0][i].e(), acc[4][i].e(1.0)+p(0,2), dotted=True)
        # bd += cell(row[i])
